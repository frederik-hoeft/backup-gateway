...
A restart does not trust any previously persisted lifecycle observation as current evidence. Startup resets configured targets to `Unknown` before reconciliation, including persisted `Starting` and `Stopping` states, because a crash may have occurred after recording a transition but before its external side effect. Reconciliation then probes every configured target and derives intent again from durable leases. Repeated Wake-on-LAN packets and repeated shutdown reconciliation are therefore expected recovery behavior rather than one-shot commands whose loss would leave the system inconsistent.
